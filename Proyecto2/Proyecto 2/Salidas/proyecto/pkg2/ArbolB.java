...
public class ArbolB {
    ...
    public void insertar(int id,Object Usuario) {
        NodoB nodo = new NodoB(id ,Usuario);
        if (raiz == null) {
            raiz = new RamaB();
            raiz.insertar(nodo);
        } else {
            NodoB obj = insertar_en_rama(nodo, raiz,Usuario);
            if (obj != null) {
        ...
        if (rama.hoja) {
            ...
            do {
                if (nodo.id == temp.id) {
                    ...
                    NodoB obj = insertar_en_rama(nodo, temp.izquierda,Usuario);
                    ...
                    NodoB obj = insertar_en_rama(nodo, temp.derecha,Usuario);
                    if (obj instanceof NodoB) {
                        rama.insertar((NodoB) obj);
                        if (rama.contador == orden_arbol) {
                            return dividir(rama,Usuario);
    ...
    private NodoB dividir(RamaB rama,Object Usuario) {
        int val = -999;
        NodoB temp, Nuevito;
